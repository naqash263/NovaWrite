...
const API_ENDPOINTS: ApiEndpoint[] = [
  ...
  },
  {
    ...
    description: 'Create a new post (Admin only) - Full control including approval status',
    auth: true,
    requestBody: {
      type: 'application/json',
      example: {
    ...
    responseExample: {
      ...
      approval_status: "approved",
      approved_by: 1,
      approved_at: "2025-10-04T10:00:00.000000Z",
      meta_description: "SEO meta description",
      meta_keywords: "keyword1, keyword2, keyword3",
      created_at: "2025-10-04T10:00:00.000000Z"
    }
...
const FIELD_VALIDATION = {
  posts: {
    title: {
      ...
      maxLength: 255,
      unique: true,
      description: 'Post title - must be unique across all posts',
    ...
    content: {
      required: true,
      type: 'string',
      description: 'Full post content in HTML format',
      example: '<h1>Introduction</h1><p>This is the main content...</p>'
    },
    excerpt: {
      required: false,
      type: 'string',
    ...
    is_published: {
      ...
      type: 'boolean',
      default: false,
      description: 'Whether the post is published and visible to public',
      example: true
    },
    approval_status: {
      required: false,
      ...
      default: 'pending',
      adminOnly: true,
      description: 'Approval status - only available in admin endpoints',
      example: 'approved'
    },
    meta_description: {
      required: false,
      ...
      example: 'This post covers important topics about...'
    },
    meta_keywords: {
      required: false,
      type: 'string',
  ...
  categories: {
    ...
    slug: {
      ...
      unique: true,
      autoGenerated: true,
      description: 'URL-friendly version of name - auto-generated if not provided',
      example: 'technology'
    },
    description: {
      required: false,
  ...
  tags: {
    ...
    description: {
      ...
      example: 'React.js related content'
    },
    color: {
      required: false,
      type: 'string',
...
?sort_by=featured,created_at&sort_order=desc,desc`}
      ...
      {activeTab === 'validation' && (
        <div className="space-y-6">
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-yellow-900 mb-2">⚠️ Important Field Constraints</h3>
            <p className="text-yellow-800">
              This section details the validation rules, required fields, and constraints for API endpoints. 
              Pay special attention to <strong>unique fields</strong> and <strong>required fields</strong> to avoid errors.
            </p>
          </div>

          {/* Posts Field Validation */}
          <div className="bg-white border rounded-lg p-6">
            <h3 className="text-xl font-semibold mb-4 text-blue-600">Posts API Field Validation</h3>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {Object.entries(FIELD_VALIDATION.posts).map(([field, rules]) => (
                    <tr key={field}>
                        ...
                        <div>
                          ...
                          {rules.example && (
                            <p className="text-xs text-gray-500 mt-1">
                              Example: <code className="bg-gray-100 px-1 rounded">{rules.example}</code>
                            </p>
                          )}
                        </div>
                      </td>
          ...
          {/* Categories Field Validation */}
          <div className="bg-white border rounded-lg p-6">
            <h3 className="text-xl font-semibold mb-4 text-green-600">Categories API Field Validation</h3>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {Object.entries(FIELD_VALIDATION.categories).map(([field, rules]) => (
                    <tr key={field}>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <code className="text-sm font-mono bg-gray-100 px-2 py-1 rounded">{field}</code>
                      </td>
          ...
          <div className="bg-white border rounded-lg p-6">
            <h3 className="text-xl font-semibold mb-4 text-purple-600">Tags API Field Validation</h3>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {Object.entries(FIELD_VALIDATION.tags).map(([field, rules]) => (
                    <tr key={field}>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <code className="text-sm font-mono bg-gray-100 px-2 py-1 rounded">{field}</code>
                      </td>
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-red-900 mb-4">🚨 Common Error Messages</h3>
            <div className="space-y-4">
              <div>
                <h4 className="font-medium text-red-800 mb-2">422 Unprocessable Entity - Validation Errors</h4>
                <div className="bg-white p-3 rounded border text-sm">
                  <p className="text-gray-700 mb-2">When required fields are missing or validation fails:</p>
  ...
  "errors": {
    ...
    "category_id": ["The category id field is required."]
              ...
              <div>
                <h4 className="font-medium text-red-800 mb-2">500 Internal Server Error - Duplicate Values</h4>
                <div className="bg-white p-3 rounded border text-sm">
                  <p className="text-gray-700 mb-2">When trying to create duplicate unique fields:</p>
                  <pre className="text-red-600">{`{
  "message": "Server Error",
  "error": "Duplicate entry 'my-title' for key 'posts_title_unique'"
}`}</pre>
                </div>
              </div>

              <div>
                <h4 className="font-medium text-red-800 mb-2">403 Forbidden - Admin Only Fields</h4>
                <div className="bg-white p-3 rounded border text-sm">
                  <p className="text-gray-700 mb-2">When using admin-only fields in public endpoints:</p>
                  <pre className="text-red-600">{`{
  "message": "Server Error",
  "error": "Field 'approval_status' not allowed in public endpoint"
}`}</pre>
                </div>
              </div>
            </div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
            ...
            <ul className="space-y-2 text-blue-800">
              <li className="flex items-start">
                ...
                <span><strong>Always check unique constraints:</strong> Use unique titles, slugs, and names to avoid conflicts</span>
              </li>
              <li className="flex items-start">
                <span className="text-blue-600 mr-2">•</span>
                <span><strong>Provide required fields:</strong> Ensure all required fields are included in your requests</span>
              </li>
              <li className="flex items-start">
                <span className="text-blue-600 mr-2">•</span>
                <span><strong>Use appropriate endpoints:</strong> Use public endpoints for basic operations, admin endpoints for advanced features</span>
              </li>
              <li className="flex items-start">
                <span className="text-blue-600 mr-2">•</span>
                <span><strong>Handle errors gracefully:</strong> Check response status codes and error messages for debugging</span>
              </li>
              <li className="flex items-start">
                <span className="text-blue-600 mr-2">•</span>
                <span><strong>Validate data before sending:</strong> Ensure data types and formats match the API requirements</span>
              </li>
            </ul>
          </div>
        </div>
