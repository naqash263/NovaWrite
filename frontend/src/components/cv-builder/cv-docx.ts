...
import { applyHiddenSections, formatDateRange, formatMonth, normalizeLayout, personalDetails, SECTION_TITLES, type CvLayout, type SectionKey } from './cv-sections';
...
export async function buildCvDocx(
  data: CVData,
  accentColor = '#1d4ed8',
  layout: { pageSize?: string; margins?: string; sections?: Partial<CvLayout> | null } = {},
): Promise<Blob> {
  const sectionLayout = normalizeLayout(layout.sections);
  data = applyHiddenSections(data, sectionLayout.hidden);
  ...
  const details = personalDetails(data)
    ...
    .join('  |  ');
  if (details) children.push(new Paragraph({ spacing: { after: 120 }, children: [new TextRun({ text: details, color: '4B5563' })] }));
  ...
  const writers: Record<SectionKey, () => void> = {
    ...
    education: () => {
      ...
      education.forEach((e) => children.push(itemLine([text(e.degree), text(e.institution)].filter(Boolean).join(', '), formatMonth(e.graduationYear))));
    },
    skills: () => {
    ...
    certificates: () => {
      ...
      certificates.forEach((c) => {
        children.push(itemLine([text(c.name), text(c.issuer)].filter(Boolean).join(', '), formatMonth(c.date)));
        if (text(c.credentialId)) children.push(plain(`Credential ID: ${text(c.credentialId)}`));
      });
    },
    achievements: () => {
      ...
      achievements.forEach((a) => {
        children.push(itemLine(text(a.title), formatMonth(a.date)));
  ...
  sectionLayout.order.forEach((key) => writers[key]());
