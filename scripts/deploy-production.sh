...
if npx vite build; then
  ...
  # Static HTML for crawlers that do not run JavaScript (served via .htaccess below)
  if node scripts/prerender.mjs; then
    echo -e "${GREEN}✅ Prerendered marketing and tool pages${NC}"
  else
    echo -e "${YELLOW}⚠️ Prerender failed - site still works as a client-rendered SPA${NC}"
...
# Drop stale prerendered pages so removed routes or a failed prerender never serve old HTML
rm -rf ~/naqashthaheem.com/public_html/prerender
...
<IfModule mod_rewrite.c>
    ...
    # Prerendered HTML for marketing and tool pages (crawlers that do not run JavaScript).
    # Generated by frontend/scripts/prerender.mjs; /about -> /prerender/about.html when it exists.
    RewriteCond %{DOCUMENT_ROOT}/prerender/index.html -f
    RewriteRule ^$ /prerender/index.html [L]
    RewriteCond %{REQUEST_URI} !^/prerender/
    RewriteCond %{DOCUMENT_ROOT}/prerender/$1.html -f
    RewriteRule ^(.+?)/?$ /prerender/$1.html [L]
